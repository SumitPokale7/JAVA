...
// does not support the inheritance of
// multiple superclasses into a single subclass. You can, as stated, create a
// hierarchy of inheritance in which a subclass
...
// SUPERCLASS ref = new SUBCLASS(); // HERE ref can only access methods which
// are available in SUPERCLASS
...
// the Box constructor to be called,
// which initializes width, height, and depth using these values. BoxWeight no
// longer initializes these values itself.
// It only needs to initialize the value unique to it: weight. This leaves Box
...
// NOTE: A superclass variable can be used to reference any object derived from
// that class.
...
// refers to the superclass of the subclass in
// which it is used.

// super.member

// Here, member can be either a method or an instance variable. This second form
// of super is most applicable to situations
// in which member names of a subclass hide members by the same name in the
// superclass.

// super( ) always refers to the constructor in the closest superclass. The
// super( ) in BoxPrice calls the constructor in
// BoxWeight. The super( ) in BoxWeight calls the constructor in Box. In a class
// hierarchy, if a superclass constructor
// requires parameters, then all subclasses must pass those parameters “up the
// line.” This is true whether or not a
...
// Because a superclass has no knowledge of any subclass, any initialization it
// needs to perform is separate from and
// possibly prerequisite to any initialization performed by the subclass.
// Therefore, it must complete its execution first.

// NOTE: If super( ) is not used in subclass' constructor, then the default or
// parameterless constructor of each superclass
// will be executed.
...
// The compiler is free to inline calls to them
// because it “knows” they will not be overridden by a subclass. When a small
// final method is called, often the Java
// compiler can copy the bytecode for the subroutine directly inline with the
...
// called late binding. However, since final
// methods cannot be overridden, a call to one can be resolved at compile time.
...
// NOTE: Declaring a class as final implicitly declares all of its methods as
// final, too.
// As you might expect, it is illegal to declare a class as both abstract and
// final since an abstract class is incomplete
// by itself & relies upon its subclasses to provide complete implementations.

// # NOTE: Although static methods can be inherited ,there is no point in
// overriding them in child classes because the
// method in parent class will run always no matter from which object you call
// it. That is why static interface methods
// cannot be inherited because these method will run from the parent interface
// and no matter if we were allowed to
