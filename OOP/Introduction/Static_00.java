...
// its class are created,
// and without reference to any object. You can declare both methods and
...
// (instance variables)
// A non-static member belongs to an instance. It's meaningless without somehow
...
// In fact, you can access a non-static member in a static context by specifying
// the object reference explicitly :
// public class Human {
...
// A static method can call only other static methods and cannot call a
...
// then the static block executes, which prints a message and then initializes b
// to a*4 or 12. Then main( ) is called,
// which calls meth( ), passing 42 to x. The three println( ) statements refer
// to the two static variables a and b,
...
// a = 3
// b = 12
// Note: main method is static, since it must be accessible for an application
...
// You cant override the inherited static methods, as in java overriding takes
// place by resolving the type of object at
// run-time and not compile time, and then calling the respective method.
// Static methods are class level methods, so it is always resolved during
// compile time.
// Static INTERFACE METHODS are not inherited by either an implementing class or
// a sub-interface.
...
// body of a non-inner class or interface T.
// Its effect is to declare that C is not an inner class. Just as a static
// method of T has no current instance of T in its
// body, C also has no current instance of T, nor does it have any lexically
// enclosing instances.
// Here, test does not have any instance of it's outer class Static. Neither
// does main.
